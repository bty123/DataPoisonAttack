...
funk_svd = Funk_SVD(n_epochs=30, n_users=int(339*(1.0 + attacksize)), n_items=5825, n_factors=35, lr=0.01, reg_rate=0.02)
funk_svd.fit(train_set, verbose=True)
rmse, mae = funk_svd.test(test_set)
# rmse_target,mae_target = funk_svd.test(test_set_target)
# rmse_random,mae_random = funk_svd.test(test_set_random)
